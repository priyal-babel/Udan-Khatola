import json
from tkinter import Entry, Label, Listbox, Scrollbar, StringVar, Tk, Toplevel, ttk, Button
...
class Booking:
    ...
    def widgets(self,top):
        ...
        Label(top, text = "Source Location: ", font=("Times New Roman", 15)).place(x = 180, y = 60) 
        ...
        Label(top, text = "Destination Location: ", font=("Times New Roman", 15)).place(x = 180, y = 100) 
        destination_chosen = ttk.Combobox(top, width = 38, textvariable = self.destination)
        ...
        Label(top, text = "Date of Journey: ", font=("Times New Roman", 15)).place(x = 180, y = 140)
        cal = DateEntry(top,selectmode="day", width=15, year=today.year, month=today.month, day=today.day, background='darkblue', foreground='white', borderwidth=2,textvariable=self.calendar)
        cal.place(x=375,y=145)

        Label(top, text = "Time of Journey: ", font=("Times New Roman", 15)).place(x = 180, y = 180) 
        time_chosen = ttk.Combobox(top, width = 10, textvariable = self.time)
        time_chosen['values'] = ['7:30','9:30','11:30','13:30','15:30','17:30','19:30','21:30','23:30']
        time_chosen.place(x=375,y=185)
        time_chosen.current()

        Label(top, text = "Class of Travel: ", font=("Times New Roman", 15)).place(x = 180, y = 220) 
        class_chosen = ttk.Combobox(top, width = 20, textvariable = self.travel_class)
        class_chosen['values'] = ['First Class','Business Class','Economy Class']
